	...
	$.ajax({url: queryURL, method: "GET"}) 
		.done(function(NYTData) {
			...
			for (var i=0; i<numArticles; i++) {
					...
					$("#articleWell-"+ articleCounter).append("<button data-link='" + NYTData.response.docs[i].web_url + "' data-pubdate='" + NYTData.response.docs[i].pub_date + "' data-title='" + NYTData.response.docs[i].headline.main + "' class='savearticle'>SAVE</button>");
		 ...
		 $('.savearticle').on('click', function(buttonData){
						alert('insideclick');
							  ...
							  $.ajax({
							    ...
							    url: '/submit',
							    data: {
							      title: buttonData.target.dataset.title,
							      pub_date: buttonData.target.dataset.pubdate,
							      article_link: buttonData.target.dataset.link
							    }
							  })
							  .done(function(data){
							    console.log("article saved properly");
							    }
							  );
						});	
		});
...
//show articles already saved in database
	function getResults(){
  // $('#results').empty();
  $.getJSON('/all', function(data) {
  	console.log('mongoarticles',data);
  	savedCounter = 0;
    for (var t = 0; t<data.length; t++){
    	savedCounter++;
    	var articleSection = $("<div>");
		articleSection.addClass('art');
		articleSection.attr('id', 'articleNum-' + savedCounter);
		$('#savedarticles').append(articleSection);

		$("#articleNum-"+ savedCounter).append('<h5>Section: ' + data[t].title + "</h5>");
		$("#articleNum-"+ savedCounter).append('<h5>' + data[t].pub_date + "</h5>");
		$("#articleNum-"+ savedCounter).append("<a href='" + data[t].article_link + "'>" + data[t].article_link + "</a>");
